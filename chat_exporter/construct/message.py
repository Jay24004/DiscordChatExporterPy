...
class MessageConstruct:
    ...
    async def build_reference(self):
        ...
        if not message.content and not message._interaction:
            message.content = "Click to see attachment"
        elif not message.content and message._interaction:
            message.content = "Click to see command"

        icon = ""
        if not message._interaction and (message.embeds or message.attachments):
            icon = DiscordUtils.reference_attachment_icon
        elif message._interaction:
    ...
    async def build_interaction(self):
        if not self.message._interaction:
            self.message._interaction = ""
            return

        user: Union[discord.Member, discord.User] = self.message._interaction.user
        ...
        self.message._interaction = await fill_out(self.guild, message_interaction, [
            ("AVATAR_URL", str(avatar_url), PARSE_MODE_NONE),
            ("BOT_TAG", is_bot, PARSE_MODE_NONE),
            ...
            ("COMMAND", "/" + self.message._interaction.name, PARSE_MODE_NONE),
            ("USER_ID", str(user.id), PARSE_MODE_NONE),
            ("INTERACTION_ID", str(self.message._interaction.id), PARSE_MODE_NONE),
        ])
    ...
    def _generate_message_divider_check(self):
        return bool(
            ...
            self.previous_message.type is not discord.MessageType.default or self.message._interaction != "" or
    ...
    async def generate_message_divider(self, channel_audit=False):
        if channel_audit or self._generate_message_divider_check():
            ...
            if self.message.reference != "" or self.message._interaction:
            ...
            self.message_html += await fill_out(self.guild, start_message, [
                ...
                ("REFERENCE", self.message.reference if self.message.reference else self.message._interaction,
                 PARSE_MODE_NONE),
                ("AVATAR_URL", str(avatar_url), PARSE_MODE_NONE),
